...
PairHbondGraphene::PairHbondGraphene(LAMMPS *lmp) : Pair(lmp)
{
...
PairHbondGraphene::~PairHbondGraphene()
{
  memory->sfree(params);
  delete [] pvector;
...
void PairHbondGraphene::compute(int eflag, int vflag)
{
  int i,j,k,m,ii,jj,kk,inum,jnum,knum,itype,jtype,ktype,iatom,imol;
  tagint tagprev;
  double rsq,rsq1,rsq2,r1,r2;
  double factor_hb,force_angle,force_kernel,evdwl,eng_lj,ehbond,force_switch;
  double c,s,c2,s2,a,b,ac,ac2,a11,a12,a22,vx1,vx2,vy1,vy2,vz1,vz2,d;
  double fi[3],fj[3],delr[3],delr1[3],delr2[3];
  double r2inv,r6inv;
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      for (kk = 0; kk < knum; kk++) {
        ...
        if (rsq < pm.cut_outersq) {
          ...
          if (ac > pm.cut_angle && ac < (MY_PI - pm.cut_angle)) {
            c2 = 2*c*c-1; //cos(2x) 
            ac2 = acos(c2);
            s = sqrt(1.0 - c*c);
            s2 = 2*s*c ;//sin(2x)
            ...
            force_kernel = r6inv*(pm.lj1*r6inv - pm.lj2)*r2inv *
              powint(c2,pm.ap);
            force_angle = pm.ap * r6inv*(pm.lj3*r6inv - pm.lj4) *
              powint(c2,pm.ap-1)*s2*2;

            eng_lj = r6inv*(pm.lj3*r6inv - pm.lj4);
            ...
            fi[0] = vx1 + b*delr[0] + d*delr[0];
            fi[1] = vy1 + b*delr[1] + d*delr[1];
            fi[2] = vz1 + b*delr[2] + d*delr[2];
            fj[0] = vx2 - b*delr[0] - d*delr[0];
            fj[1] = vy2 - b*delr[1] - d*delr[1];
            fj[2] = vz2 - b*delr[2] - d*delr[2];
...
void PairHbondGraphene::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairHbondGraphene::settings(int narg, char **arg)
{
...
void PairHbondGraphene::coeff(int narg, char **arg)
{
  if (narg < 6 || narg > 10)
...
void PairHbondGraphene::init_style()
{
  ...
  for (int m = 0; m < nparams; m++) {
    params[m].lj1 = 48.0*params[m].epsilon*pow(params[m].sigma,12.0);
    params[m].lj2 = 24.0*params[m].epsilon*pow(params[m].sigma,6.0);
    params[m].lj3 = 4.0*params[m].epsilon*pow(params[m].sigma,12.0);
    params[m].lj4 = 4.0*params[m].epsilon*pow(params[m].sigma,6.0);
...
double PairHbondGraphene::init_one(int i, int j)
{
  int m;
...
double PairHbondGraphene::single(int i, int j, int itype, int jtype,
                                   double rsq,
                                   ...
                                   double &fforce)
{
  ...
  double rsq1,rsq2,r1,r2,c,s,c2,s2,ac,ac2,r2inv,r6inv,factor_hb;
  double switch1,switch2;
  ...
  for (kk = 0; kk < knum; kk++) {
    ...
    if (ac < pm.cut_angle || ac > (MY_PI - pm.cut_angle)) return 0.0;
    c2 = 2*c*c-1; //cos(2x) 
    ac2 = acos(c2);
    s = sqrt(1.0 - c*c);
    s2 = 2*s*c ;//sin(2x)
    ...
    r2inv = 1.0/rsq;
    r6inv = r2inv*r2inv*r2inv;
    force_kernel = r6inv*(pm.lj1*r6inv - pm.lj2)*r2inv * powint(c2,pm.ap);
    force_angle = pm.ap * r6inv*(pm.lj3*r6inv - pm.lj4) *
      powint(c2,pm.ap-1)*s2*2;
    ...
    fforce += force_kernel*powint(c2,pm.ap) + eng_lj*force_angle;
    eng += eng_lj * powint(c2,pm.ap) * factor_hb;
  }
